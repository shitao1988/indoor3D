...
L.Popup = L.Class.extend({
	...
	_adjustPan: function () {
		...
		if (containerPos.y - dy-180 - paddingTL.y < 0) { // top
			dy = containerPos.y - paddingTL.y-180;
		}
...
L.Circle = L.Path.extend({
	...
	getPoints: function () {
	    var angle = Math.PI * ((1 / this.options.sides) - (1 / 2));
	    var centerPixel = this._map.latLngToLayerPoint(this._latlng);
	    var points = [];
	    ...
	    for (var i = 0; i < this.options.sides; i++) {
	        var rotatedAngle = angle + (i * 2 * Math.PI / this.options.sides);
	        var x = centerPixel.x + (this._mRadius * Math.cos(rotatedAngle)/500);
	        var y = centerPixel.y + (this._mRadius * Math.sin(rotatedAngle)/500);

	        points.push(this._map.layerPointToLatLng(L.point(x, y)));
	    }
	...
	L.Control.Zoomslider = (function () {
		...
		var Zoomslider = L.Control.extend({
			...
			_createUI: function () {
				...
				ui.tip =L.DomUtil.create('div', 'leaflet-bar-tip', ui.bar);
				ui.zoomOut = this._createZoomBtn('out', 'bottom', ui.bar);
				ui.body    = L.DomUtil.create('div', ns + '-body', ui.wrap);
				ui.knob    = L.DomUtil.create('div', ns + '-knob');


                ui.tip1 = L.DomUtil.create('div', ns + '-tip1', ui.tip);
                ui.tip2 = L.DomUtil.create('div', ns + '-tip2', ui.tip);
                ui.tip3 = L.DomUtil.create('div', ns + '-tip3', ui.tip);
                ui.tip4 = L.DomUtil.create('div', ns + '-tip4', ui.tip);
			...
			_initEvents: function () {
				...
				L.DomEvent.on(this._ui.wrap,    'mouseover', this._onSliderHover, this);
				L.DomEvent.on(this._ui.tip,    'mouseover', this._onSliderHover, this);
				L.DomEvent.on(this._ui.wrap,    'mouseout', this._onSliderMouseOut, this);
				L.DomEvent.on(this._ui.zoomIn,  'click', this._zoomIn,        this);
				L.DomEvent.on(this._ui.zoomOut, 'click', this._zoomOut,       this);
				L.DomEvent.on(this._ui.tip1, 'click', this._zoom1,this);
				L.DomEvent.on(this._ui.tip2, 'click', this._zoom2,this);
				L.DomEvent.on(this._ui.tip3, 'click', this._zoom3,this);
				L.DomEvent.on(this._ui.tip4, 'click', this._zoom4,this);

				this._knob.on('dragend', this._updateMapZoom, this);
			},
			_onSliderHover: function (e) {
			this._ui.tip.style.display='inline';
			},
			_onSliderMouseOut:function(e){
               this._ui.tip.style.display='none';
			},
            ...
            _zoom1: function (e) {
				this._map.setView([29.8627,121.556], 16);
			},
			_zoom2: function (e) {
				this._map.setView([29.8627,121.556], 11);
			},
			_zoom3: function (e) {
			  this._map.setView([29.8627,121.556], 7);
			},
			_zoom4: function (e) {
				 this._map.setView([29.8627,104.556], 4);
			},
			_zoomIn: function (e) {
				this._map.zoomIn(e.shiftKey ? 3 : 1);
			},
...
L.Control.ComputeDist = L.Control.extend({
    _rad: Math.PI / 180,
    options: {
        position: "topright"
    },
    onAdd: function(map) {
        ...
        var div = L.DomUtil.create("div", "imap-control-computeDist storage-control");
        var link = L.DomUtil.create('a', '', div);
        link.href = '#';
        link.title = "\u6d4b\u8ddd";
        L.DomEvent.disableClickPropagation(div);
        L.DomEvent.on(link, "click", this._onMouseClick, this);
        return div;
    },
    onRemove: function(map) {
        map._distControl = null;
    },
...
L.Control.ComputeArea = L.Control.extend({
    _rad: Math.PI / 180,
    _deg: 180 / Math.PI,
    options: {
        position: "topright"
    ...
    onAdd: function(map) {
        ...
        var div = L.DomUtil.create("div", "imap-control-computeArea storage-control");
        var link = L.DomUtil.create('a', '', div);
        link.href = '#';
        link.title = "\u6d4b\u9762\u79ef";
        L.DomEvent.disableClickPropagation(div);
    ...
    _setArea: function(event) {
        latlng = event.latlng || this._map.mouseEventToLatLng(event);
