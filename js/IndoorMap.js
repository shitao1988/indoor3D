...
WktParser.wkt2Array = function(wkt) {
    ...
    if (result.type) {
        result.coords = WktParser._getCoords(wkt.substring(result.type.length));
...
WktParser._getCoords = function(wkt) {
    wkt = wkt.replace(/\(/g, "[");
    wkt = wkt.replace(/\)/g, "]");
    wkt = wkt.trim();
...
(function() {
    var a = "ActiveXObject" in window,
        ...
        y = "devicePixelRatio" in window && 1 < window.devicePixelRatio || "matchMedia" in window && window.matchMedia("(min-resolution:144dppi)") && window.matchMedia("(min-resolution:144dppi)").matches,
...
Rect.prototype.isCollide = function(rect) {
    if (rect.br[0] < this.tl[0] || rect.tl[0] > this.br[0] || rect.br[1] < this.tl[1] || rect.tl[1] > this.br[1]) {
        return false;
    }
    return true;
}
...
function Mall() {
    ...
    this.showFloor = function(id) {
            if (_this.is3d) {
                ...
                for (var i = 0; i < _this.floorWalls.length; i++) {
                    _this.root.remove(this.floorWalls[i]);
                }
    ...
    this.showAllFloors = function(scale) {
        ...
        var offset = 4;
        scale = isNaN(parseFloat(scale)) ? 1 : parseFloat(scale);
        scale = Math.max(scale, 0.5);
        var totalHeight = 0;
        for (var i = 0; i < _this.floors.length; i++) {
            totalHeight += _this.floors[i].height * offset * scale;
        }
        var baseHeight = -totalHeight / 2;
        for (var i = 0; i < _this.floors.length; i++) {
            _this.floors[i].position.set(0, 0, i * _this.floors[i].height * offset * scale + baseHeight);
            //            if(i == 4){
    ...
    this.showFloorWalls = function() {
            ...
            //_this.root.remove(_this.building);
            for (var i = 0; i < _this.floors.length; i++) {
                _this.root.remove(this.floors[i]);
            }
            var offset = 4;
            var totalHeight = 0;
            for (var i = 0; i < _this.floors.length; i++) {
                totalHeight += _this.floors[i].height * offset;
            }
            var baseHeight = -totalHeight / 2;
            for (var i = 0; i < _this.floorWalls.length; i++) {
                _this.floorWalls[i].position.set(0, 0, i * _this.floors[i].height * offset + baseHeight);
                //            if(i == 4){
                //                _this.floors[i].position.set(0,-300,i*_this.floors[i].height*offset);
                //            }else{
                //
                //            }
                _this.root.add(this.floorWalls[i]);
            }
            //this.building.scale.set(1,1,offset);
            _curFloorId = 0;
            return _this.root;
        }
        /*this.selectCell = function(floorId, cellId){
            //var this.getCurFloor();
            var curFloorId = this.getCurFloorId();
            
            if(floorId !== curFloorId){
                this.showFloor(floorId);
            }
            
            var floorObj = this.getCurFloor();
            
            if(floorObj.cell){
                var cell = floorObj.cell[cellId];
...
var default3dTheme = {
        name: "test", //theme's name
        background: "#eff1f2", //background color
        //building's style
        building: {
            color: "#000000",
            opacity: 0.1,
...
function ParseModel(json, is3d, theme, sceneSize) {
    ...
    function parse() {
        ...
        for (var i = 0; i < json.data.floors.length; i++) {
            ...
            if (is3d) { // for 3d model
                ...
                //create walls of current floor
                shape = buildShapes(points);
                var floorWallObj = new THREE.Object3D();
                for (var k = 0; k < shape.length; k++) {
                    //solid model
                    extrudeSettings = {
                        amount: floorHeight * 4,
                        bevelEnabled: false
                    };
                    geometry = new THREE.ExtrudeGeometry(shape[k], extrudeSettings);
                    material = new THREE.MeshLambertMaterial(theme.floorWall(i));
                    mesh = new THREE.Mesh(geometry, material);
                    mesh.type = "floorwall";
                    mesh.id = floor.id;
                    mesh.dataInfo = floor;
                    floorWallObj.add(mesh);
                    //top wireframe
                    ...
                    floorWallObj.add(wire);
                }
                mall.floorWalls.push(floorWallObj);
            } else { //for 2d model
            ...
            if (is3d) {
                ...
                for (var m = 0; m < floor.pubPoint.length; m++) {
                    ...
                    //console.log("pubpoint:" + m + ",end");
                }
            }
            //console.log("floor:" + i + ",end");
        }
        if (is3d) {
            ...
            if (basePoints.length > 0) {
                ...
                if (shape.length === 1) {
                    ...
                    for (var p = 0; p < shape.length; p++) {
                        ...
                        geometry = new THREE.ExtrudeGeometry(shape[p], extrudeSettings);
                        mesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial(theme.building));
    ...
    function parseWKT(wktString) {
        ...
        if (!baseCenter) {
            var baseRect = getRect(basePoints);
            baseCenter = {
                x: (baseRect.minx + baseRect.maxx) / 2,
                y: (baseRect.miny + baseRect.maxy) / 2
            };
        }
        for (var j = 0; j < coordsLength; j++) {
            ...
            for (var i = 0; i < pointArray[j].length; i++) {
                ...
                for (var k = 0; k < pointArray[j][i].length; k++) {
                    var pointObj = coordsToScene(sceneRealRatio, {
                        x: pointArray[j][i][k][1] - baseCenter.x,
                        y: pointArray[j][i][k][0] - baseCenter.y
                    });
    ...
    function getWKTPoints(wktString) {
        var parseResult = WktParser.wkt2Array(wktString),
            pointArray = [];
    ...
    function getSceneRealRatio(realBbox, sceneSize) {
        ...
        return {
            width: sceneRealWidthRatio,
            height: sceneRealHeightRatio
        };
    }

    function coordsToScene(sceneRealRatio, point) {
        return {
            x: point.x * sceneRealRatio.width,
            y: point.y * sceneRealRatio.height
        };
    }
...
IndoorMap.getUI = function(indoorMap) {
    ...
    _uiRoot = document.getElementById("louceng");
    var floors = _indoorMap.mall.jsonData.data.Floors;
    for (var i = 0; i < floors.length; i++) {
        (function(arg) {
            li = document.createElement('li');
            li.value = arg;
            text = document.createTextNode(floors[arg].Name + "层");
            li.appendChild(text);
            //li.onclick = function () {
            //    }
